...
| 이름 | 역할 |
|---|---|
| jiwan | 전체 시스템 아키텍처 설계, Vision AI 모델 개발 및 데이터 파이프라인 구축, 로봇 제어 핵심 로직 및 안전 시스템 구현 |
| Junhyung | 로봇 제어 세부 로직 및 동작 시퀀스 설계, TCP 기반 회전 동작 및 명령 통신 구현, ROS2 node 관리 및 시스템 연동 |
| Sooyoung | HMI 전체 UI/UX 설계 및 구현, 시스템 관리자 인터페이스 개발, 로봇 수동 제어 및 모션 제어 탭 구현 |
| Minseong | 프로젝트 아이디어 제안 및 요구사항 정의, 개발 일정 관리 및 문서화, 테스트 계획 수립 및 성능 평가 |
